...
def rand_subset(path, n, tgt_only=True):
    lines = []
    m = 0
    for line in open(path, encoding='utf-8'):
...
if __name__ == "__main__":
    fld = 'data/victoriana_vs_other/'
    #shuffle_file(false.txt')
    #mix_shuffle(fld + 'true.txt.shuffled', fld + 'false.txt.shuffled', fld, n=2e6, prob_T=0.1, repeat=False, tgt_only=True)
	
    for name in ['vali','test','train']:
        txt2num(fld + '/%s.txt'%name, fld + '/vocab.txt', tgt_only=True)